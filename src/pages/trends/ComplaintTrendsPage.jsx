  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (latestData.status === 'fulfilled') {
        ...
        setTrends(all.filter((e) => e.type === 'COMPLAINT_SPIKE'));
      }
      if (trendsData.status === 'fulfilled') setDashboardTrends(trendsData.value);
    } catch (err) {
      setError('Failed to load complaint trends.');
    } finally {
      setLoading(false);
    }
  }, [window]);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-bg-card rounded-xl border border-border p-5 shadow-xs">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-text-muted uppercase tracking-wide">Complaint Spikes</span>
          </div>
        ...
        <div className="bg-bg-card rounded-xl border border-border p-5 shadow-xs">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-text-muted uppercase tracking-wide">Total Occurrences</span>
          </div>
          <p className="text-2xl font-semibold text-text-primary">{totalComplaints}</p>
        </div>
        <div className="bg-bg-card rounded-xl border border-border p-5 shadow-xs">
          <div className="flex items-center gap-2 mb-2">
