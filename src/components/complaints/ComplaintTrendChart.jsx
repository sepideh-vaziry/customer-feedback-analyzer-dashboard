...
export default function ComplaintTrendChart({ data, loading }) {
  if (loading) {
    return (
  ...
  if (labels.length === 0 || Object.keys(series).length === 0) {
    return (
      <div className="bg-bg-card rounded-xl border border-border p-6 shadow-xs">
        <h3 className="text-base font-semibold text-text-primary mb-4">Complaint Trends</h3>
        <EmptyState icon={TrendingUp} title="No trend data" description="Complaint trends will appear as feedback is analyzed over time." />
      </div>
    );
  }
