  ...
  if (!data || data.totalAnalyzed === 0) {
    return (
      <div className="bg-bg-card rounded-xl border border-border p-6 shadow-xs">
        <h3 className="text-base font-semibold text-text-primary mb-4">Sentiment Distribution</h3>
        ...
        {chartData.map((item) => {
          ...
          return (
            <div key={item.key} className="text-center p-3 rounded-lg bg-bg-base border border-border">
              <Icon size={16} className="mx-auto mb-1" style={{ color: COLORS[item.key] }} />
              <p className="text-lg font-semibold text-text-primary">{percent}%</p>
